...
export class TextTokenizer {
  ...
  processEncoding(encoding) {
    return {
      ...
      tokens: this.tokenizer.decode(encoding.input_ids, { skip_special_tokens: true }),
  ...
  padBatch(encodings, maxLength) {
    const paddedEncodings = encodings.map(encoding => {
      if (encoding.inputIds.length < maxLength) {
        const padding = new Array(maxLength - encoding.inputIds.length).fill(this.tokenizer.pad_token_id);
        return {
          ...encoding,
          inputIds: [...encoding.inputIds, ...padding],
          attentionMask: [...encoding.attentionMask, ...new Array(padding.length).fill(0)]
        };
      }
      return encoding;
    });
  ...
  calculateBatchPaddingStats(encodings) {
    const paddingRatios = encodings.map(enc => 
      this.calculatePadding(enc.attentionMask).paddingRatio
    );

    return {
      averagePaddingRatio: paddingRatios.reduce((a, b) => a + b, 0) / paddingRatios.length,
